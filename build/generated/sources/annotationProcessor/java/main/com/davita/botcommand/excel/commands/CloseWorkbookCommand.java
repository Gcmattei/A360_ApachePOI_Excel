...
public final class CloseWorkbookCommand implements BotCommand {
  private static final Logger logger = LogManager.getLogger(CloseWorkbookCommand.class);
  ...
  public Optional<Value> execute(GlobalSessionContext globalSessionContext,
    ...
    CloseWorkbook command = new CloseWorkbook();
    HashMap<String, Object> convertedParameters = new HashMap<String, Object>();
    if(parameters.containsKey("saveChanges") && parameters.get("saveChanges") != null && parameters.get("saveChanges").get() != null) {
      convertedParameters.put("saveChanges", parameters.get("saveChanges").get());
      if(convertedParameters.get("saveChanges") !=null && !(convertedParameters.get("saveChanges") instanceof Boolean)) {
        throw new BotCommandException(MESSAGES_GENERIC.getString("generic.UnexpectedTypeReceived","saveChanges", "Boolean", parameters.get("saveChanges").get().getClass().getSimpleName()));
      }
    }
    if(convertedParameters.get("saveChanges") == null) {
      throw new BotCommandException(MESSAGES_GENERIC.getString("generic.validation.notEmpty","saveChanges"));
    }
    ...
    try {
      command.action((Boolean)convertedParameters.get("saveChanges"),(WorkbookSession)convertedParameters.get("session"));Optional<Value> result = Optional.empty();
